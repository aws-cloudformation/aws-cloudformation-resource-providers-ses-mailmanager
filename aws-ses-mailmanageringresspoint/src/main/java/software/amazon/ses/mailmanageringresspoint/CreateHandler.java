...
public class CreateHandler extends BaseHandlerStd {
    ...
    private boolean stabilizedOnCreate(
        ...
        switch (status) {
            case "ACTIVE":
                logger.log(String.format("[ClientRequestToken: %s] IngressPoint %s is stabilized, current state is %s", clientRequestToken, ingressPointId, status));
                return true;
            case "PROVISIONING":
                logger.log(String.format("[ClientRequestToken: %s] IngressPoint %s is stabilizing, current state is %s", clientRequestToken, ingressPointId, status));
                return false;
            default:
                String exceptionMsg = String.format("[ClientRequestToken: %s] IngressPoint %s reached unexpected state %s", clientRequestToken, ingressPointId, status);
                logger.log(exceptionMsg);
                throw new CfnNotStabilizedException(ResourceModel.TYPE_NAME, exceptionMsg);
        }
    }
}
