...
public class UpdateHandlerTest extends AbstractTestBase {
    ...
    private ResourceModel desired_AddonInstance_model;
    private ResourceModel previous_AddonInstance_model;
    ...
    public void setup() {
        ...
        desired_AddonInstance_model = ResourceModel.builder()
                .addonInstanceId(AddonInstance_ID)
                .addonSubscriptionId(AddonSubscription_ID)
                .build();

        previous_AddonInstance_model = ResourceModel.builder()
                .addonInstanceId(AddonInstance_ID)
                .addonSubscriptionId(AddonSubscription_ID)
                .build();
    }
    ...
    @Test
    @Tag("SkipTearDown")
    public void handle_request_simple_failure_due_to_update_addon_sub_id() {
        final UpdateHandler handler = new UpdateHandler();

        desired_AddonInstance_model.setAddonSubscriptionId("Another_Id");
    ...
    public void handle_request_simple_success_with_tagging_only() {
        ...
        desired_AddonInstance_model.setTags(
                List.of(
                        software.amazon.ses.mailmanageraddoninstance.Tag.builder().key("KeyOne").value("valueOne").build(),
                        software.amazon.ses.mailmanageraddoninstance.Tag.builder().key("KeyTwo").value("valueTwo").build()
                )
        );
        ...
        assertThat(response.getResourceModel().getAddonInstanceId()).isEqualTo(request.getDesiredResourceState().getAddonInstanceId());
    ...
    public void handle_request_simple_success_with_tagging_and_untagging() {
        ...
        desired_AddonInstance_model.setTags(
                List.of(
                        software.amazon.ses.mailmanageraddoninstance.Tag.builder().key("key_1").value("value_1").build(),
                        software.amazon.ses.mailmanageraddoninstance.Tag.builder().key("key_2").value("new_value_2").build(),
                        software.amazon.ses.mailmanageraddoninstance.Tag.builder().key("key_3").value("value_3").build()
                )
        );

        previous_AddonInstance_model.setTags(
                List.of(
                        software.amazon.ses.mailmanageraddoninstance.Tag.builder().key("key_1").value("value_1").build(),
                        software.amazon.ses.mailmanageraddoninstance.Tag.builder().key("key_2").value("value_2").build(),
                        software.amazon.ses.mailmanageraddoninstance.Tag.builder().key("key_4").value("value_4").build()
                )
        );
    ...
    public void handle_request_failure_due_to_resource_not_found() {
        ...
        when(mailManagerClient.getAddonInstance(any(GetAddonInstanceRequest.class))).thenThrow(ResourceNotFoundException.builder().message("Resource not found").build());
        final ProgressEvent<ResourceModel, CallbackContext> response = handler.handleRequest(proxy, request, new CallbackContext(), proxyClient, logger);
