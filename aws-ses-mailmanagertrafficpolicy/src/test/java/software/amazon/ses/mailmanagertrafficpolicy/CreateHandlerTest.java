...
public class CreateHandlerTest extends AbstractTestBase {
    ...
    public void handle_request_simple_success_with_tags() {
        ...
        ArgumentCaptor<CreateTrafficPolicyRequest> captor = ArgumentCaptor.forClass(CreateTrafficPolicyRequest.class);

        verify(mailManagerClient).createTrafficPolicy(captor.capture());
        assertThat(captor.getValue().trafficPolicyName()).isEqualTo(TRAFFIC_POLICY_NAME);
        assertThat(captor.getValue().tags().size()).isEqualTo(4);

        assertThat(response).isNotNull();
        assertThat(response.getStatus()).isEqualTo(OperationStatus.SUCCESS);
        assertThat(response.getCallbackDelaySeconds()).isEqualTo(0);
        assertThat(response.getResourceModel().getTrafficPolicyId()).isNotNull();
        assertThat(response.getResourceModel().getTrafficPolicyArn()).isNotNull();
        assertThat(response.getResourceModel().getTags()).isNotNull();
        assertThat(response.getResourceModel().getTags().size()).isEqualTo(2);
        assertThat(response.getResourceModels()).isNull();
        assertThat(response.getMessage()).isNull();
